 ...
 */
final class CachingCombinedResourceInfo {

	// Constants ------------------------------------------------------------------------------------------------------

	private static final Logger logger = Logger.getLogger(CachingCombinedResourceHandler.class.getName());

	// ConcurrentHashMap was considered, but duplicate inserts technically don't harm and a HashMap is faster on read.
	private static final Map<String, CachingCombinedResourceInfo> CACHE = new HashMap<String, CachingCombinedResourceInfo>();
	 ...
	 */
	private CachingCombinedResourceInfo(String id, Set<ResourceIdentifier> resourceIdentifiers) {
		this.id = id;
	 ...
	 */
	public static CachingCombinedResourceInfo get(String id) {
		CachingCombinedResourceInfo info = CACHE.get(id);

		if (info == null) {
			Set<ResourceIdentifier> resourceIdentifiers = fromUniqueId(id);

			if (resourceIdentifiers != null) {
				info = new CachingCombinedResourceInfo(id, Collections.unmodifiableSet(resourceIdentifiers));
				CACHE.put(id, info);
			}
		}
	...
	/**
	 * Returns true if the given object is also an instance of {@link CachingCombinedResourceInfo} and its ID equals to the
	 * ID of the current combined resource info instance.
	 */
	@Override
	public boolean equals(Object other) {
		return (other instanceof CachingCombinedResourceInfo)
			? ((CachingCombinedResourceInfo) other).id.equals(id)
			: false;
	}
