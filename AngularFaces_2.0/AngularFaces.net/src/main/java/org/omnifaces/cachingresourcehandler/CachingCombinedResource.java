 ...
 */
final class CachingCombinedResource extends Resource {

	// Properties -----------------------------------------------------------------------------------------------------

	private CachingCombinedResourceInfo info;

	// Constructors ---------------------------------------------------------------------------------------------------

	/**
	 * Constructs a new combined resource based on the given resource name. This constructor is only used by
	 * {@link CachingCombinedResourceHandler#createResource(String, String)}.
	 * @param name The resource name of the combined resource.
	 */
	public CachingCombinedResource(String name) {
		String[] resourcePathParts = name.split("\\.", 2)[0].split("/");
		String resourceId = resourcePathParts[resourcePathParts.length - 1];
		info = CachingCombinedResourceInfo.get(resourceId);
		setResourceName(name);
		setLibraryName(CachingCombinedResourceHandler.LIBRARY_NAME);
	 ...
	 * is only used by {@link CachingCombinedResourceHandler#handleResourceRequest(FacesContext)}.
	 ...
	 * this exception is handled by {@link CachingCombinedResourceHandler#handleResourceRequest(FacesContext)} which should
	 * return a 404.
	 */
	public CachingCombinedResource(FacesContext context) {
	...
	public String getRequestPath() {
		...
		return Faces.getRequestContextPath()
			...
			+ "?ln=" + CachingCombinedResourceHandler.LIBRARY_NAME
	...
	public InputStream getInputStream() throws IOException {
		if (!info.getResources().isEmpty()) {
			return new CachingCombinedResourceInputStream(info.getResources());
		}
		else {
			return null;
		}
