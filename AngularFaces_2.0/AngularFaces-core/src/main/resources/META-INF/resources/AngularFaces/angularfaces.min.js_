...
'de.beyondjava.angularfaces.behavior.event':'ngsync',execute:ids,
render:'angular'});
}
}catch(e){
...
if($scope.jqueryField.hasClass("pristine")){ return "true"; }
return "false";
};
...
if($($scope.jqueryField).is(":visible")){ return ""; }
return "hidden";
};
...
if($scope.jqueryField.hasClass("ng-invalid")){ return "ui-state-error"; }
return "";
};
$scope.visibilityClass=function(){
if($($scope.jqueryField).is(":visible")){ return ""; }
...
$scope.message=function(){
if(typeof ($scope.messages)=="undefined"||$scope.messages==null){ return ""; }
return $scope.messages.detail;
};
$scope.hasMessage=function(){
if(typeof ($scope.messages)=="undefined"||$scope.messages==null){ return false; }
return true;
};
$scope.visibilityClass=function(){
if(typeof ($scope.messages)=="undefined"||$scope.messages==null){ return "hidden"; }
return "";
};
$scope.getTemplate=function(){
...
alert("Couldn't restore the field values. ngModel="+model+" element="+element
+" Exception="+e);
}
}
try{
...
console.log(e);
alert("injectVariableIntoScope(model, value): Couldn't inject variable "+model
+" into scope.\n"+value+"="+value+"\n"+e);
}
}
...
if(!reactivateAngularJS){ return; }
if(data.source.type!="submit"){ return; }
switch(data.status){
case "begin":
break;
case "complete":
