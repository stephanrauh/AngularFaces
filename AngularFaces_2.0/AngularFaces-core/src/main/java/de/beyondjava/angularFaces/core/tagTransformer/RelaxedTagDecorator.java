...
public class RelaxedTagDecorator implements TagDecorator {
	...
	private static enum Mapper {
		body("h:body"), head("h:head"), label("h:outputLabel"), form("h:form"), button(
				new ElementConverter("h:button", "jsf:outcome"), new ElementConverter("h:commandButton"));
	...
	private static class ElementConverter implements TagDecorator {
		...
		public Tag decorate(Tag tag) {
			...
			// PENDING
			/**
			 * if (!arbiterAttribute.isLiteral()) { // TODO should we throw an
			 * exception here? }
			 **/
