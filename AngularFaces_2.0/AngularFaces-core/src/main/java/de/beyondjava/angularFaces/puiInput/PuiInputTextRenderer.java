...
public class PuiInputTextRenderer extends HtmlBasicInputRenderer {
    ...
    public PuiInputTextRenderer() {
        LOGGER.fine(getClass().getName() + " is being initialized");
    }
    ...
    public void encodeBegin(FacesContext context, UIComponent component) throws IOException {
        ...
        renderNonEmptyAttribute(input, html, "label", input.getLabel());
        renderNonEmptyAttribute(input, html, "value", ELTools.getCoreValueExpression(input));
        NGBeanAttributeInfo infos = ELTools.getBeanAttributeInfos(input);
        if (infos.isHasMin()) {
            renderNonEmptyAttribute(input, html, "min", String.valueOf(infos.getMin()));
        }
        if (infos.isHasMax()) {
            renderNonEmptyAttribute(input, html, "max", String.valueOf(infos.getMin()));
        }
        if (infos.isHasMinSize()) {
            renderNonEmptyAttribute(input, html, "minlength", String.valueOf(infos.getMin()));
        }
        if (infos.isHasMaxSize()) {
            renderNonEmptyAttribute(input, html, "maxlength", String.valueOf(infos.getMin()));
        }
        if (infos.isRequired()) {
            renderNonEmptyAttribute(input, html, "required", "true");
        }
        html.append(">");
        html.append("</pui-input>");
     ...
     */
    private void renderNonEmptyAttribute(PuiInput input, StringBuffer html, final String attibuteName,
