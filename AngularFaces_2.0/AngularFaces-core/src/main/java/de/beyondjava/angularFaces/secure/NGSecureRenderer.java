...
/**
 * This component analyzes user input to prevent security breaches. Note that you are still responsible for your
...
public class NGSecureRenderer extends HtmlBasicInputRenderer {
    ...
    private NGSecurityFilter findAndVerifySecurityFilter(FacesContext context, UIComponent component)
        ...
        if (null != checkedBy) {
            try {
                ...
                .append("<div style=\"color:#F00\">Configuration error: security class filter could not be instantiated</div>");
            }
            catch (IllegalAccessException e) {
                context.getResponseWriter()
                .append("<div style=\"color:#F00\">Configuration error: security class filter has been forbidden to be instantiated</div>");
            }
    ...
    private void showLegalDisclaimer(FacesContext context, NGSecurityFilter filter) throws IOException {
        if (filter.getClass() == NGDefaultSecurityFilter.class) {
            context.getResponseWriter()
            .append("<div style=\"color:#F00\">Warning: This application is only protected by AngularFaces default security.<br /> This may be better than no protection, but it must not be used in a production environment.</div>");
        }
    }
}
