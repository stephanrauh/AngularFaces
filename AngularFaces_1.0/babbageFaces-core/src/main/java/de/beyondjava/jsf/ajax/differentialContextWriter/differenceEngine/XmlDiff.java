...
public class XmlDiff {
	 ...
	 * @param newHTMLTags
	 */
	private static void assertTagIDMatch(List<HTMLTag> oldHTMLTags,
	...
	public static boolean attributesAreEqualOrCanBeChangedLocally(
			HTMLTag oldHTMLTag, HTMLTag newHTMLTag,
			List<String> attributeChanges) {
		if (!oldHTMLTag.hasAttributes() && !newHTMLTag.hasAttributes()) {
			return true;
		}

		// if (oldHTMLTag.getAttributes().size() >
		// newHTMLTag.getAttributes().size()) {
		// return GLOBAL_CHANGE_REQUIRED;
		// }

		// Have attributes been deleted?
		boolean attributeChangeMustBeFixedGlobally = hasAnAttributeBeenDeleted(
				oldHTMLTag, newHTMLTag, attributeChanges);
	 ...
	 *         identical.
	 */
	private static boolean childHTMLTagsAreEqualOrCanBeChangedLocally(
		...
		if (insertList.size() > 0) {
			...
			if (!needsUpdate) {
				needsUpdate |= generateInsertCommands(newHTMLTags,
						localInserts, insertList, localUpdates);
			}
		}
		if (!needsUpdate) {
			List<HTMLTag> deleteList = getRecentlyAddedHTMLTags(newHTMLTags,
					oldHTMLTags);
	 ...
	 */
	private static boolean generateInsertCommands(List<HTMLTag> newHTMLTags,
			...
			for (int index = 0; index < parent.getChildren().size(); index++) {
				...
				if (insert == sibling) {
					...
					while (((index - offset) >= 0)
							&& "script".equals(parent.getChildren()
									.get(index - offset).getNodeName())) {
						offset++;
						if ((index - offset) < 0) {
							isScriptNode = true;
							break;
					...
					if (!isScriptNode && ((index - offset) >= 0)) {
						final String idOfSibling = parent.getChildren()
								.get(index - offset).getId();
						if ((null != idOfSibling) && (idOfSibling.length() > 0)) {
							s = "<insert id=\"" + idOfNewTag
									+ "\"><after id=\"" + idOfSibling
									+ "\"><![CDATA[" + temporaryDiv
									+ "]]></after></insert>";
						}
					}
					...
					if (null == s) {
						// try to insert the new node before a node bearing an
						// id
						offset = 1;
						...
						final String idOfSibling = parent.getChildren()
								.get(index + offset).getId();
						if ((null == idOfSibling)
								|| (idOfSibling.length() == 0)) {
							return true;
						}
						s = "<insert id=\"" + idOfNewTag + "\"><before id=\""
								+ idOfSibling + "\"><![CDATA[" + temporaryDiv
								+ "]]></before></insert>";
					}
					inserts.add(s);
					// needed to fix a Mojarra bug
	...
	private static boolean hasAnAttributeBeenDeleted(HTMLTag oldHTMLTag,
			HTMLTag newHTMLTag, List<String> changes) {
		{
			HTMLAttribute newAttribute = null;
			HTMLAttribute oldAttribute = null;
			for (int i = 0; i < oldHTMLTag.getAttributes().size(); i++) {
				...
				if (null == newAttribute) {
					if ((newHTMLTag.getId() != null)
							&& (newHTMLTag.getId().length() > 0)) {
						String s = "<eval><![CDATA[document.getElementById('"
								+ newHTMLTag.getId() + "').removeAttribute('"
								+ attributeName + "');]]></eval>";
						changes.add(s);
					} else {
						// we cannot fix this attribute change locally, the
						// entire
