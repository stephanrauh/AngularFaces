...
public class DifferenceEngine {
	 ...
	 */
	private List<HTMLTag> determineNecessaryChangeFromResponse(HTMLTag change,
			HTMLTag lastKnownDOMTree) {

		if (change.getNodeName().equals("update")) {
			String id = change.getId();
			HTMLTag changingHTML;
			if (change.getFirstChild().isCDATANode()
					|| change.getFirstChild().isTextNode()) {
				changingHTML = new HTMLTag(change.getFirstChild()
						.getInnerHTML().toString().trim());
			} else {
				changingHTML = new HTMLTag(change.getFirstChild().toString()
						.trim());
			}

			HTMLTag lastKnownCorrespondingHTMLTag = lastKnownDOMTree
					.findByID(id);
			if (null == lastKnownCorrespondingHTMLTag) {
				LOGGER.severe(lastKnownDOMTree.toString());
				LOGGER.severe("Couldn't find id " + id
						+ " in the last known DOM tree");
				throw new IllegalArgumentException("Couldn't find id " + id
						+ " in the last known DOM tree");
			}
			return determineNecessaryChanges(changingHTML,
					lastKnownCorrespondingHTMLTag);
		} else {
	 ...
	 * @return
	 */
	private List<HTMLTag> determineNecessaryChanges(HTMLTag changingHTML,
			HTMLTag lastKnownCorrespondingHTMLTag) {
		List<String> deletions = new ArrayList<>();
		List<String> attributeChanges = new ArrayList<>();
		List<String> inserts = new ArrayList<>();
		List<HTMLTag> updates = new ArrayList<>();
		determineNecessaryChanges(changingHTML, lastKnownCorrespondingHTMLTag,
		...
		if (null != updates) {
			for (HTMLTag n : updates) {
				...
				HTMLTag partialChangeHTMLTag = new HTMLTag("update", partialID,
						partialUpdate);
				partialChanges.add(partialChangeHTMLTag);
			}
		}
	 ...
	 * @param changes2
	 */
	protected List<HTMLTag> determineNecessaryChanges(HTMLTag newDOM,
			HTMLTag lastKnownCorrespondingHTMLTag, List<HTMLTag> updates,
			List<String> deletions, List<String> attributeChanges,
			List<String> inserts) {
		XmlDiff.tagsAreEqualOrCanBeChangedLocally(
				lastKnownCorrespondingHTMLTag, newDOM, updates, deletions,
	...
	/**
	 * Removes a script from an update response command. The script is added to
	 * the hash map. If there's HTML content in the command, it is appended to
	 * the StringBuffer.
	 * 
	 ...
	 */
	private void exciseScriptsFromUpdateCommand(
			Map<String, String> scriptsToBeAdded, StringBuffer replacement,
		...
		if (scriptTagStart > 0) {
			...
			if (scriptIDEnd > 0) {
				...
				if ((scriptStart > 0) && (scriptEnd > 0)) {
					scriptsToBeAdded.put(
							c.substring(scriptIDStart, scriptIDEnd),
							c.substring(scriptStart + 2, scriptEnd));
					c = c.substring(0, scriptTagStart)
							+ c.substring(scriptEnd + "</script>".length());
					if (c.contains("<![CDATA[]]>")) {
						isPureJSTag = true;
					}
				}
	 ...
	 * @return
	 */
	private List<HTMLTag> extractChangesFromPartialResponse(
			...
			if (group.startsWith("<update")) {
				...
				if (end > start) {
					...
					if ((start >= 0) && (end > start)) {
						String html = group.substring(
								start + "<![CDATA[".length(), end);
						try {
							HTMLTag update = new HTMLTag("<update id=\"" + id
									+ "\">" + html + "</update>");
	...
	/**
	 * Sometimes PrimeFaces uses non-standard responses that can't be optimized
	 * by BabbageFaces without interfering with PrimeFaces. For instance,
	 * paginated datatables use non-standard responses.
	 * 
	 * @param change
	 * @return
	 */
	private boolean isRegularUpdateCommand(HTMLTag change) {
		if (change.getChildren().size() != 1) {
			return false;
		} else if ((change.getFirstChild().getId() == null)
				|| "".equals(change.getFirstChild().getId())) {
			return false;
		} else {
			return true;
		}
	 ...
	 * @return
	 */
	private String optimizeResponse(String currentResponse,
			HTMLTag domTreeToBeUpdated, HTMLTag change,
			List<HTMLTag> newPartialChanges) {
		String id = change.getId();
		return optimizeResponse(currentResponse, domTreeToBeUpdated,
				newPartialChanges, id);
	}

	/**
	 * This method replaces one of the update commands of the response with the
	 * sequence of optimized commands (that has been calculated before).
	 * 
	 * @param currentResponse
	 ...
	 */
	private String optimizeResponse(String currentResponse,
			HTMLTag domTreeToBeUpdated, List<HTMLTag> newPartialChanges,
			String id) {
		if (!id.contains("javax.faces.ViewState")) {
			Map<String, String> scriptsToBeAdded = new HashMap<>();
			Map<String, HTMLTag> scriptsInOriginalDomTree = domTreeToBeUpdated
					.collectScripts();
			...
			String currentResponseEnd = currentResponse.substring(end
					+ "</update>".length());
			String currentResponseStart = currentResponse.substring(0, start);
			StringBuffer replacement = new StringBuffer();
			for (HTMLTag changeDefinition : newPartialChanges) {
				exciseScriptsFromUpdateCommand(scriptsToBeAdded, replacement,
						changeDefinition);
				String idOfCurrentChange = changeDefinition.getId();
				if ((idOfCurrentChange == null)
						|| (idOfCurrentChange.length() == 0)) {
					...
					HTMLTag currentChangeTag = domTreeToBeUpdated
							.findByID(idOfCurrentChange);
					if (null != currentChangeTag) {
						...
						for (HTMLTag scriptNode : requiredScripts) {
							if (!scriptsToBeAdded.containsKey(scriptNode
									.getId())) {
								scriptsToBeAdded.put(scriptNode.getId(),
										scriptNode.getFirstChild()
												.toCompactString());
							}
						}
					}
			...
			if ((replacement.length() < lengthOfOriginalCommand)
	 ...
	 * @return
	 */
	private HTMLTag retrieveLastKnownHTMLFromSession(
			Map<String, Object> sessionMap) {
		HTMLTag html = (HTMLTag) sessionMap.get(LAST_KNOWN_HTML_KEY);
		if (html == null) {
			throw new RuntimeException(
					"There's no HTML code stored in the session");
		}
		return html;
	}
	 ...
	 * @param idToBeUpdated
	 */
	private void updateHTMLTag(HTMLTag domTreeInSession, HTMLTag newSubtree,
		...
		if (tagToBeReplaced == null) {
			if (!idToBeUpdated.contains("javax.faces.ViewState")) { // JSF omits
				...
				// AJAX
				// responses
				LOGGER.severe("Wrong ID? Looking for "
						+ idToBeUpdated
	 ...
	 * @return
	 */
	public String yieldDifferences(String currentResponse,
			Map<String, Object> sessionMap, boolean isAJAX) {
		...
		if (isAJAX) {
			...
			for (HTMLTag change : listOfChanges) {
				if (change.getNodeName().equals("update")) {
					...
					if (change.getId().equals("javax.faces.ViewRoot")) {
						...
						int endOfFirstUpdate = currentResponse
								.indexOf("</update>");
						String secondUpdate = currentResponse
								.substring(endOfFirstUpdate);
						...
						if (headerAsString.equals(oldHeaderAsString)) {
							...
							if ((startNew >= 0) && (endNew >= 0)) {
								title = headerAsString.substring(startNew
										+ "<title>".length(), endNew);
								headerAsString = headerAsString.substring(0,
										startNew)
							...
							if ((startOld >= 0) && (endNew >= 0)) {
								oldHeaderAsString = oldHeaderAsString
										.substring(0, startOld)
							...
							if (headerAsString.equals(oldHeaderAsString)) {
								...
								s += "<update id=\"javax.faces.ViewHead\"><![CDATA["
										+ headerAsString + "]]></update>";
								optimizedUpdates++;
							}
						}
						s += "<update id=\"javax.faces.ViewBody\"><![CDATA["
								+ body.toCompactString() + "]]>";
						s += secondUpdate;
						currentResponse = s;

						List<HTMLTag> newBodyChanges = determineNecessaryChanges(
								body, domTreeToBeUpdated.findTag("body"));
						...
						currentResponse = optimizeResponse(currentResponse,
								domTreeToBeUpdated, newBodyChanges,
					...
					} else if ((!change.getId().contains(
							"javax.faces.ViewState"))
							&& ((!change.getId().contains(
									"javax.faces.ViewHead")))
							&& ((!change.getId().contains(
									"javax.faces.ViewBody")))) {
						if (isRegularUpdateCommand(change)) {
							List<HTMLTag> newPartialChanges = determineNecessaryChangeFromResponse(
									change, domTreeToBeUpdated);
							if ((null != newPartialChanges)) {
								currentResponse = optimizeResponse(
										currentResponse, domTreeToBeUpdated,
										change, newPartialChanges);
							...
							updateHTMLTag(domTreeToBeUpdated,
									change.getFirstChild(), change.getId());
						}
					}

				} else if ((!"error-name".equals(change.getNodeName()))
						&& (!"error-message".equals(change.getNodeName()))) {
					LOGGER.severe("Unexpected JSF response ("
							+ change.getNodeName() + ")");
					originalOtherTags++;
				} else {
					originalErrorTags++;
			...
			boolean isPrimefacesResponse = currentResponse
					.contains("javax.faces.resource/primefaces.js");
			final HTMLTag responseWithAdditionalIDs = new HTMLTag(
					currentResponse);
			sessionMap.remove(LAST_KNOWN_HTML_KEY);
			sessionMap.put(LAST_KNOWN_HTML_KEY, responseWithAdditionalIDs);
			String body = responseWithAdditionalIDs.getChildren().get(1)
					.toString();
			...
			if ((bodyIndex > 0) && (bodyEndIndex > 0)) {
				currentResponse = currentResponse.substring(0, bodyIndex)
						...
						+ body
						+ currentResponse.substring(bodyEndIndex
								+ "</body>".length());
			}
			int pos = currentResponse
					.indexOf("<div id=\"babbageFacesStatistics\"></div>");
			if (pos > 0) {
		...
		if (isAJAX) {
			responseMessage = "<table><tr><td></td><td ><b>Original response</b></td><td><b>Optimized response</b></td></tr>"
					...
					+ optimizedLength
					+ " bytes</td><td>"
					+ (((100 * optimizedLength) / originalLength))
					+ "%</td></tr>";
			responseMessage += "<tr><td>total </td><td> "
					+ DEBUG_originalBytesCumulated + "</td><td> "
			...
			responseMessage += "<tr><td>inserts:</td><td></td><td> "
					+ optimizedInserts + "</td></tr>";
			responseMessage += "<tr><td>deletes:</td><td></td><td> "
					+ optimizedDeletes + "</td></tr>";
			responseMessage += "<tr><td>attributes:</td><td></td><td> "
					+ optimizedAttributes + "</td></tr>";
			responseMessage += "<tr><td>Original error tags</td><td>"
					+ originalErrorTags + "</td></tr>";
			responseMessage += "<tr><td>other original tags</td><td>"
					+ originalOtherTags + "</td></tr></table>";
			int pos = currentResponse.indexOf("</changes>");
			if (pos > 0) {
				currentResponse = currentResponse.substring(0, pos)
						+ "<update id=\"babbageFacesStatistics\"><![CDATA[<div id=\"babbageFacesStatistics\">"
						+ responseMessage + "</div>]]></update>"
						+ currentResponse.substring(pos);
			}

		} else {
			responseMessage = "HTML - original response:  " + originalLength
					+ " bytes  Optimized response: " + optimizedLength
					+ " bytes  total original: " + DEBUG_originalBytesCumulated
