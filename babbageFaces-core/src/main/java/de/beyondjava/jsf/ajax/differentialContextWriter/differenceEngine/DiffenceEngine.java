    ...
    */
	...
	private List<HTMLTag> determineNecessaryChangeFromResponse(HTMLTag change,
		...
		if (change.getNodeName().equals("update")) {
			...
			String changingHTML = change.getFirstChild().getInnerHTML()
					.toString().trim();
			...
			determineNecessaryChanges(changingHTML,
					lastKnownCorrespondingHTMLTag, updates, deletions, changes,
					inserts);
			...
			if (null != updates) {
				for (HTMLTag n : updates) {
					...
					String partialChange = "<update id=\"" + partialID
							+ "\"><![CDATA[" + partialUpdate + "]]></update>";
	 ...
	 * @param changes2
	 */
	protected List<HTMLTag> determineNecessaryChanges(String newHTML,
...
//				lastKnownCorrespondingHTMLTag, updates);
	...
	private List<HTMLTag> extractChangesFromPartialResponse(
			HTMLTag partialResponseAsDOMTree) {
	...
	private HTMLTag getLastKnownHTMLBodyAsDomTree(Map<String, Object> sessionMap) {
		...
		HTMLTag lastKnownDOMTree = new HTMLTag("<original>" + html
				+ "</original>");
		...
		for (HTMLTag afterOrBefore : children) {
			...
			for (int i = 0; i < parent.getChildren().size(); i++) {
				if (parent.getChildren().get(i) == sibling) {
					final HTMLTag elementToBeInserted = new HTMLTag(
							afterOrBefore.getFirstChild().getInnerHTML()
									.toString());
	 ...
	 * @return
	 */
	private String retrieveLastKnownHTMLFromSession(
	 ...
	 * @param c
	 */
	private void updateAttributes(HTMLTag domTreeInSession, String HTMLTagid,
			String c) {
			...
			if (atAll) {
				...
				for (HTMLTag change : listOfChanges) {
					List<HTMLTag> newPartialChanges = determineNecessaryChangeFromResponse(
							change, updatedDOMTree);
					if ((null != newPartialChanges)) {
						String id = change.getId();
						int start = currentResponse.indexOf("<update id=\""
								+ id + "\">");
						int end = currentResponse.indexOf("</update>", start);
						String currentResponseEnd = currentResponse
								.substring(end + "</update>".length());
						try {
							String tmpCurrentResponse = currentResponse
									.substring(0, start);
							for (HTMLTag changeDefinition : newPartialChanges) {
								String c = changeDefinition.toCompactString();
								tmpCurrentResponse += c;
								String idOfCurrentChange = changeDefinition
										.getId();
								if (changeDefinition.getNodeName().equals(
										"insert")) {
									insertHTMLTag(updatedDOMTree,
											changeDefinition.getChildren());
								} else if ((idOfCurrentChange == null)
										|| (idOfCurrentChange.length() == 0)) {
									LOGGER.severe("Missing HTMLTag ID");
								} else if (changeDefinition.getNodeName()
										.equals("update")) {
									updateHTMLTag(updatedDOMTree,
											changeDefinition.getFirstChild(),
											idOfCurrentChange);
								} else if (changeDefinition.getNodeName()
										.equals("delete")) {
									deleteHTMLTag(updatedDOMTree,
											idOfCurrentChange);
								} else if (changeDefinition.getNodeName()
										.equals("attributes")) {
									updateAttributes(updatedDOMTree,
											idOfCurrentChange, c);
								}

							}
							currentResponse = tmpCurrentResponse
									+ currentResponseEnd;
						} catch (Exception e) {
							LOGGER.log(
									Level.SEVERE,
									"An exception occured while trying to replace the response",
									e);
						}
					}
					...
					if (newHTML.startsWith("<original>")) {
						...
						newHTML = newHTML.substring(0, newHTML.length()
								- "</original>".length());
					}
