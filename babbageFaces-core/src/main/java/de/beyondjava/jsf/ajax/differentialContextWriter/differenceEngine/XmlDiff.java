...
public class XmlDiff {
    ...
    *         ADDED_LOCAL_CHANGE, if the tags differ, but it is not necessary to
    *         exchange the entire tag. NO_CHANGE_REQUIRED, if both tags are
    *         identical.
    * 
    * @param oldHTMLTag
    * @param newHTMLTag
   ...
   /**
    * XML parsers tend to produce empty HTML tags. This method strips empty HTML
    * tags from a list of HTML tags.
    * 
    * @param tags
    * @return
    */
    ...
    * old version to the new version of the tags. These changes are returned via
    * the call by reference parameters "updates", "deletions", "inserts" and
    * "attributeChanges". Sometimes the transformation requires the exchange of
    * the whole tag. If so, the method returns GLOBAL_CHANGE_REQUIRED. <br>
    * Note the similarity between the two methods
    * childHTMLTagsAreEqualOrCanBeChangedLocally() and
    * tagsAreEqualOrCanBeChangedLocally(). There need to be two methods because
    * the first method recognizes the difference between two lists, while the
   ...
   public static boolean tagsAreEqualOrCanBeChangedLocally(HTMLTag oldHTMLTag, HTMLTag newHTMLTag,
      ...
      boolean childHTMLTagHaveChanged = !childHTMLTagsAreEqualOrCanBeChangedLocally(oldHTMLTags, newHTMLTags, updates,
